...
| `reasoning-effort-budget.patch` | Maps supported Chat Completions and Responses effort values onto ROCmplete's bounded reasoning policy and forwards thinking efforts to templates such as Qwen3.8 that implement model-native effort instructions. | Both endpoints honor no-thinking and the advertised low, medium, and high budgets without the patch, including the configured exhaustion message, and recognized effort values reach `chat_template_kwargs`. Related upstream work is tracked in [PR 20479](https://github.com/ggml-org/llama.cpp/pull/20479). |
